...
suspend fun postReport(client: HttpClient, report: Report){
    ...
    client.post("$baseurl/api/report") {
        ...
        when (response.status) {
            HttpStatusCode.Created -> println("Report created successfully")
            else -> println("Failed to create report: ${response.status}")
        }
    }

    println("Report posted successfully")
}
...
suspend fun deleteReport(client: HttpClient, id: Long) {
    client.delete("$baseurl/api/reports/$id"){
        ...
        when (response.status) {
            ...
            else -> println("Failed to delete report: ${response.status}")
        }
    }
}
